...
const RecordsScreen = () => {
  ...
  const contains = ({ day }, query) => {
    if (day.includes(query)) {
      return true;
    }
  ...
  const fetchData = () => {
    setRefreshing(true);
    fetch('http://localhost:3000/residentRecords/?user_id=' + userId ,
    {
    	method: "GET",
        headers: {
    ...
    const renderItem = ({ item }) => (
        <UserRecordCell recordDate={item.day} bloodPreaseure={item.blood_presure} respiratoryRate={item.respiration_rate}
